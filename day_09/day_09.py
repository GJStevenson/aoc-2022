...
def main():
   ...
   head_direction_col = {'L': -1, 'U': 0, 'R': 1, 'D': 0}
   head_direction_row = {'L': 0, 'U': -1, 'R': 0, 'D': 1}
   ...
   for (direction, amount) in commands:
      for _ in range(amount):
         head = (head[0] + head_direction_row[direction], head[1] + head_direction_col[direction])
...
def adjust(head, tail):
    drow = (head[0] - tail[0])
    dcolumn = (head[1] - tail[1])
    if abs(drow) <= 1 and abs(dcolumn) <= 1:
        pass
    elif abs(drow) >= 2 and abs(dcolumn) >= 2:
